      ...
      <div class="zm-product-mobileBox" v-for="(item,index) in setMobLists" :key="index" :data-sortid="item.id" style="display:block;">
        <div class="zm-product-imgBox">
          <div style="width:100%;height:100%;overflow:hidden;">
            <a class="zm-product-img" :href="item.url" target="_blank" :style="{backgroundImage:'url('+item.mainImage+')'}">
            </a>
        ...
        <div class="zm-product-cont" style="padding:10px 20px;">
          <a class="zm-product-title" :href="item.url" target="_blank" style="max-height: 34px;">{{item.name}}</a>
          <a class="zm-product-price" :href="item.url" target="_blank" :style="{display:settingData.priceShow.num}">{{item.priceStr}}
          </a>
          <a class="zm-product-details" :href="item.url" target="_blank" :style="{display:settingData.skuShow.num,backgroundColor:'#3098d5'}">
            查看详情
          </a>
        </div>
      </div>
